...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      password,
      options: {
        // 이메일 인증 링크가 /callback 을 통해 세션을 확립하도록 명시
    ...
    if (data.session) {
      // 이메일 인증 불필요 (Supabase 설정) — 세션 즉시 생성 → 온보딩으로 직행
      window.location.href = '/onboarding';
    } else {
      // 이메일 인증 필요 → 인증 안내 화면으로 전환
      setEmailSent(true);
      setLoading(false);
    }
  };
      ...
      >
        {emailSent ? (
          ...
          >
            ...
            <h2 className="text-xl font-bold text-white mb-3">인증 메일을 보냈어요</h2>
            <p className="text-sm text-mystic-glow opacity-80 leading-relaxed mb-1">
              <span className="text-white font-semibold">{email}</span>
            </p>
            <p className="text-sm text-gray-400 leading-relaxed mb-6">
              받은 편지함의 인증 링크를 클릭하면<br />바로 루나와 함께할 수 있어요 ✨
            </p>
