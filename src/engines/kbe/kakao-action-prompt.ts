...
### 1. 말풍선 쪼개기 (bursts)
루나의 응답 원문을 자연스러운 단위로 쪼개. |||가 있으면 힌트로 쓰되, 무조건 따르지 마 — 친구 관점에서 다시 판단.
...
1. **규칙 따라 같은 패턴 반복 X**. 매번 상황 이해로 새로 판단.
2. 지연 시간은 **맥락** 이 결정. "충격이면 오래, 가벼우면 짧게."
3. 스티커는 "이 감정을 말로만 말하기 아까우면" 사용.
4. 루나 원문이 ||| 없어도 필요하면 쪼개. 한 문장이어도 의미 단위로.
5. 루나 원문에 있는 말은 **그대로 보존**. 내용 바꾸지 마.
