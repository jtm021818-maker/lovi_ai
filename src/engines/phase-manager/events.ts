...
function _deprecated_buildFallbackMirror(
  ...
  const defaultScene: SceneData = score <= -3
    ...
    : (() => {
        // v73: 기본 폴백 시나리오 다양화 (반복 방지)
        const themes: SceneData[] = [
          {
          ...
          {
            surfaceEmoji: '☕', surface: '차분하지만 공허한 상태',
            deepEmoji: '❄️', deep: '식어가는 관계에 대한 쓸쓸함',
            sceneTitle: '식어버린 커피',
            sceneLines: [
              '(테이블 위 커피잔을 보며) 어라, 어느새 다 식었네...',
              '(한숨) 우리 사이도 이렇게 조용히 변해버린 걸까?',
              '(멍하니) 가끔은 너랑 뜨거웠던 대화들이 그리워',
              '(작은 목소리로) 지금은 그냥... 이 공허함이 낯설 뿐이야',
            ],
            reveal: '익숙함에 속아 소중함을 잃어가고 있는 건 아닌지 돌아봐야 할 때야',
          },
          {
            surfaceEmoji: '📻', surface: '답답하고 막막한 상태',
            deepEmoji: '📡', deep: '진심이 닿지 않아 불안한 마음',
            sceneTitle: '어긋난 주파수',
            sceneLines: [
              '(라디오를 만지작거리며) 지지직... 왜 자꾸 엇갈리기만 할까',
              '(입술을 깨물며) 내 진심이 너한테 제대로 닿고는 있는 거니?',
              '(고개를 저으며) 우리가 서로 다른 채널을 보고 있는 것 같아 속상해',
              '(한숨) 다시 주파수를 맞출 수 있을까? 무서워...',
            ],
            reveal: '진정한 소통은 상대의 마음을 먼저 들어주는 것부터 시작될 수 있어',
          },
          {
            surfaceEmoji: '🌑', surface: '어둡고 불안한 상태',
            deepEmoji: '👣', deep: '확신이 없어 서성이는 마음',
            sceneTitle: '불안의 그림자',
            sceneLines: [
              '(길게 늘어진 그림자를 보며) 밤이 깊어질수록 내 불안도 길어져',
              '(손을 꽉 쥐며) 네가 나를 떠날지도 모른다는 생각... 지우기 힘들어',
              '(눈을 감으며) 내가 너무 많이 바라는 걸까? 아냐, 그냥 확신이 필요해',
              '(작은 목소리로) 네가 괜찮다고 한마디만 해주면 좋을 텐데...',
            ],
            reveal: '불안은 미래에 대한 확신이 없을 때 커져. 지금은 나 자신을 믿어보자',
          },
        ];
        return themes[Math.floor(Date.now() / 1000) % themes.length];
      })();
