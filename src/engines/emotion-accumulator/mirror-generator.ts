...
라벨 = 성별 한글. 유저 성별은 시스템이 지정함: userGender="${userGender ?? '미확인'}". [${userGenderLabel}]/[${partnerGenderLabel}] 사용. 등장인물이 유저 혼자면 solo.
⚠️ characterSetup.userGender 는 반드시 "${userGender ?? 'female'}" 로 출력할 것. 대화 내용에서 추측하지 말 것.
...
## sceneFrames (표정)
각 대사마다 표정 0~8 (배열로). revealFrame 도 하나.
0=기본 1=슬픔 2=화남 3=깨달음 4=놀람 5=약간웃음/행복 6=불안 7=당당 8=매우행복/환희(눈물 날 정도로 기쁠 때만)

행복 구분:
- 조금 좋은 일, 설렘, 미소 → 5 (약간웃음)
- 소식 듣자마자 심장 터질 것 같은 기쁨, 감격, 환희 → 8 (매우행복)
...
→ 위 재료로 장면이 그려지면 ready=true + JSON. 아예 재료 0이면 ready=false. 재료 조금만 있어도 네 상상으로 채워서 ready=true.`;
  ...
  try {
    ...
    // 전달된 userGender 우선 (LLM이 대화 내용에서 잘못 추측하는 걸 방지)
    const llmUserGender: 'male' | 'female' | undefined =
      (userGender === 'male' || userGender === 'female') ? userGender :
